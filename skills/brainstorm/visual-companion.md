...
**Use the conversation** when the content is text or tabular. Route eligible
2-3 option questions through the host-native structured-question tool as
...
1. **Check server is alive**, then **write HTML** to a new file in `screen_dir`:
   ...
   - Use your file-creation tool — **never use cat/heredoc** (dumps noise into command output)
...
3. **On your next turn** — after the user responds in the conversation:
   ...
   - Merge with the user's conversational response to get the full picture
   - The conversational response is the primary feedback; `state_dir/events` provides structured interaction data
...
If `$STATE_DIR/events` doesn't exist, the user didn't interact with the browser — use only their conversational response.
