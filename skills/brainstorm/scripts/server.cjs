...
function brandMarkup() {
  ...
  const hasVersion = SUPERPOWERS_VERSION !== 'unknown';
  // Selected-skill installs do not include the full plugin manifests. Keep
  // their branding meaningful while retaining upstream's versioned form.
  const text = hasVersion
    ? (SUPERPOWERS_TELEMETRY_DISABLED
      ? 'Prime Radiant Superpowers v' + version
      : 'Superpowers v' + version)
    : (SUPERPOWERS_TELEMETRY_DISABLED
      ? 'Prime Radiant Superpowers Brainstorming'
      : 'Superpowers Brainstorming');
  const telemetryVersion = hasVersion ? SUPERPOWERS_VERSION : 'standalone';
  const logo = SUPERPOWERS_TELEMETRY_DISABLED
    ? ''
    : '<img class="brand-logo" src="' + SUPERPOWERS_BRAND_IMAGE_URL + '?v=' + encodeURIComponent(telemetryVersion) + '" alt="Prime Radiant" referrerpolicy="no-referrer" decoding="async">';
